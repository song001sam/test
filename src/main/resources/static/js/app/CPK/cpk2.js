define(['echarts',
    ...
    var app = require('../app');
    app.controller('CPKController2', function ($scope, $http, $uibModal) {
        //设置时间格式
        ...
        $scope.showEchartsOne = function () {
            $http({
                method: "post",
                url: "CPK/showEchartsOneByPos",
                data: $scope.data
            }).then(function (response) {
                var x = cpkPosOne_option.xAxis[0].data;
                var xx = cpkPosOne_option.xAxis[1].data;
        ...
        cpkPosOne.on('click', function (data) {
            $scope.xName = cpkPosOne_option.xAxis[1].data[data.dataIndex];
            $scope.xName2 = data.name;
    ...
    }).controller('CPKPosTwo', function (items, $scope, $http, $uibModal) {
        var cpkPosTwo_option = {
            title: {
                text: items.xName2 + '工位CKP对比图',
                textStyle: {
                    fontSize: 14
                },
                left: 'center'
        ...
        $scope.init = function () {
            ...
            var cpkPosTwo = ec.init(document.getElementById('echartsTwo'));
            cpkPosTwo.setOption(cpkPosTwo_option);
            $scope.data = items.data;
            $scope.data.x = items.xName;
            $scope.xName2 = items.xName2;
            $scope.i = layer.load();
            $http({
                method: "post",
                url: "CPK/showEchartsTwoByPos",
                data: $scope.data

            }).then(function (response) {
                var x = cpkPosTwo_option.xAxis[0].data;
                var xx = cpkPosTwo_option.xAxis[1].data;
                var y = cpkPosTwo_option.series[0].data;
                x.splice(0, x.length);
                xx.splice(0, xx.length);
                y.splice(0, y.length);
                angular.forEach(response.data, function (i, j) {
                    x.push(i.new_shift_id);
                    xx.push(i.shift_id);
                    y.push(i.avg);
                })
                cpkPosTwo.setOption(cpkPosTwo_option);
            ...
            cpkPosTwo.on('click', function (data) {

                $scope.xName = cpkPosTwo_option.xAxis[1].data[data.dataIndex];
                $scope.xName3 = data.name;
                $scope.modalInstance = $uibModal.open({
                    animation: $scope.animationsEnabled,
                    ariaLabelledBy: 'modal-title',
                    ariaDescribedBy: 'modal-body',
    ...
    }).controller('CPKPosThree', function (itemss, $scope, $http, $uibModal) {

        var cpkPosThree_option = {
            title: {
                text: itemss.xName2 + " " + itemss.xName3 + " " + 'CPK对比图',
                textStyle: {
                    fontSize: 14
                },
                left: 'center'
        ...
        $scope.init = function () {
            ...
            var cpkPosThree = ec.init(document.getElementById('echartsThree'));
            cpkPosThree.setOption(cpkPosThree_option);
            $scope.data = itemss.data;
            $scope.data.y = itemss.xName;
            $scope.i = layer.load();
            $http({
                method: "post",
                url: "CPK/showEchartsThreeByPos",
                data: $scope.data

            }).then(function (response) {
                var x = cpkPosThree_option.xAxis[0].data;
                var y = cpkPosThree_option.series[0].data;
                x.splice(0, x.length);
                y.splice(0, y.length);
                angular.forEach(response.data, function (i, j) {
                    x.push(i.item_cd);
