define(['echarts',
    ...
    var app = require('../../app');
    app.controller('dcyqhlController1', function ($scope, $location) {
//		$scope.list = [];
//		$scope.totalItems = 0;
//		$scope.currentPage = 1;
//		var $ctrl = this;
...
// 		}
        ...
        var ws = "ws://" + host + ":" + port + "/BDIA/ws/bof1_2";
        ...
        if (typeof (WebSocket) == "undefined") {
            ...
            socket.onmessage = function (msg) {
                ...
                var dcyqhl_1 = parseFloat(str[9]);

                var data_dcyqhl_0 = $scope.option1.series[0].data;
                if (data_dcyqhl_0.length < 12) {//图标展示4个点
                    //option.xAxis[0].data.push(axisData); //在数组后添加一项（x轴）
                    //data0.push(num);//在数组最后加入数据 （y轴）
                    $scope.option1.xAxis[0].data.push(tim); //在数组后添加一项（x轴）
                    data_dcyqhl_0.push(dcyqhl_1);//在数组最后加入数据 （y轴）
                } else {
                    data_dcyqhl_0.shift();//移除数组的第一项 （y轴）
                    data_dcyqhl_0.push(dcyqhl_1);//在数组最后加入数据 （y轴）
